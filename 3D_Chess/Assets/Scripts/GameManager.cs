...
public class GameManager : MonoBehaviour
{
    ...
    private Dictionary<Square, ChessPiece> squareToPiece = new();
    ...
    {
        chessGame = new Game();
        RegisterAllPiecesAtStart();
    }

    private void RegisterAllPiecesAtStart()
    {
        ChessPiece[] pieces = FindObjectsOfType<ChessPiece>();
        foreach (var piece in pieces)
        {
            var sq = piece.logicalSquare;  // Awake에서 startFile/startRank로 설정됨 :contentReference[oaicite:5]{index=5}
            squareToPiece[sq] = piece;
        }
    }

    public void OnPieceMoved(ChessPiece piece, Square from, Square to)
    {
        squareToPiece.Remove(from);
        squareToPiece[to] = piece;
    }

    public bool TryGetPiece(Square square, out ChessPiece piece)
        => squareToPiece.TryGetValue(square, out piece);


    
    public bool TryMoveByVoice(Square from, Square to, out string error)
    {
        error = null;

        if (!TryGetPiece(from, out ChessPiece piece))
        {
            error = $"No piece on {from}.";
            return false;
        }

        bool success = piece.SetSquare(to);  
        ...
        var snap = piece.GetComponent<SnapToBoardOnRelease>();
        if (snap != null)
        {
            snap.MoveToSquareInstant(to);
        }
